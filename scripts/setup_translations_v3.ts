...
async function setupTranslationsV3() {
    let conn;
    try {
        conn = await pool.getConnection();
        console.log("Connected to database.");

        // 1. Drop old tables (v2 tables)
        const dropQueries = [
        ...
        for (const item of items) {
            ...
            if (item.name) {
                await conn.query(
                    `INSERT INTO items_translations (item_id, field_type, language_code, base, base_language_code, value) VALUES (?, 1, 'de', ?, 'de', ?)`,
            ...
            if (item.description) {
                await conn.query(
                    `INSERT INTO items_translations (item_id, field_type, language_code, base, base_language_code, value) VALUES (?, 2, 'de', ?, 'de', ?)`,
        ...
        for (const cat of categories) {
             ...
             if (cat.name) {
                await conn.query(
                    `INSERT INTO itemcats_translations (category_id, field_type, language_code, base, base_language_code, value) VALUES (?, 1, 'de', ?, 'de', ?)`,
            ...
            if (cat.description) {
                await conn.query(
                    `INSERT INTO itemcats_translations (category_id, field_type, language_code, base, base_language_code, value) VALUES (?, 2, 'de', ?, 'de', ?)`,
             ...
             if (cat.additional_text) {
                await conn.query(
                    `INSERT INTO itemcats_translations (category_id, field_type, language_code, base, base_language_code, value) VALUES (?, 3, 'de', ?, 'de', ?)`,
        ...
        try {
            ...
            for (const zuta of zutaten) {
                ...
                if (zuta.name) {
                    await conn.query(
                        `INSERT INTO pizza_zutaten_translations (goods_id, field_type, language_code, base, base_language_code, value) VALUES (?, 1, 'de', ?, 'de', ?)`,
