...
async function run() {
    ...
    try {
        ...
        console.log("\nClearing api_keys and pc_clients tables...");
        ...
        // Use TRUNCATE to clear data and reset autoincrement (requires no FK constraint issues, or handled sequentially)
        // Since pc_clients references api_keys, we clear pc_clients first.
        console.log("Clearing pc_clients...");
        await conn.query("DELETE FROM pc_clients");
        await conn.query("ALTER TABLE pc_clients AUTO_INCREMENT = 1");
        
        console.log("Clearing api_keys...");
        await conn.query("DELETE FROM api_keys");
        await conn.query("ALTER TABLE api_keys AUTO_INCREMENT = 1");
